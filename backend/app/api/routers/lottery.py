...
# from backend.app.schemas.lottery import DrawResultResponse, HotNumber, HistoryResponse
...
# async def get_history(limit: int = 10, offset: int = 0, db: AsyncSession = Depends(get_session)):
...
# async def get_hot_numbers(periods: int = 30, db: AsyncSession = Depends(get_session)):
#     query = text("""
#         WITH recent AS (
